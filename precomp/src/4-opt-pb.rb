...
#
# Usage:
#   (1) generate .opb file:        ruby 4-opt.rb 3.txt 4.opb
#   (2) invert .sol file to .txt: ruby 4-opt.rb 3.txt 4.sol 4.opb.map
#
...
class Node
  ...
  def constraint
    ...
    lhs = children.map {|c| "+1 " + c.var }.join(" ")
    # if this node is pinned, var is always 1
    @pinned ? "#{ lhs } >= #{ weight }" : "#{ lhs } #{ term(weight, negate: true) } >= 0"
  end
...
class DAG
  ...
  def dump_opb(lp)
    ...
    @start_nodes.each_with_index do |n|
      constraints << "+1 #{ n.var } = 1" unless n.pinned
    end
    each_node do |n|
    ...
    nv = vars.map{|v| /^x(\d+)$/ =~ v; $1.to_i }.max

    gen = NameGenerator.new
    open(lp, "w") do |f|
      f.puts "* #variable= #{nv} #constraint= #{constraints.size}"
      f.puts("min: " + obj.join(" ") + ";")

      f.puts constraints.map {|c| c + ";" }.join("\n")
    end
    puts "OPB size: #{ File.size(lp).to_s.reverse.scan(/.{1,3}/).join(",").reverse }"
...
if ARGV.size < 2
  puts "usage:"
  puts "  ruby #$0 3.txt 4.opb"
  puts "  ruby #$0 3.txt 4.sol 4.opb.map"
  exit
end

dag = DAG.new(ARGV[0])
if File.extname(ARGV[1]) == ".opb"
  dag.optimize
  dag.dump_lp(ARGV[1])
else
